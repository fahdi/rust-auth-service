...
impl AuthDatabase for PostgresDatabase {
    ...
    async fn count_verified_users(&self) -> Result<u64, UserError> {
        let query = "SELECT COUNT(*) as count FROM users WHERE email_verified = true";
        let row = sqlx::query(query)
            .fetch_one(&self.pool)
            .await
    ...
    async fn count_active_users(&self) -> Result<u64, UserError> {
        ...
        let query =
            "SELECT COUNT(*) as count FROM users WHERE last_login >= $1 AND is_active = true";
        let row = sqlx::query(query)
    ...
    async fn search_users(
        ...
        let sql = r#"
            SELECT * FROM users 
            WHERE email ILIKE $1 OR first_name ILIKE $2 OR last_name ILIKE $3
    ...
    async fn update_user_role(&self, user_id: &str, role: &str) -> Result<(), UserError> {
        let query = "UPDATE users SET role = $1, updated_at = $2 WHERE user_id = $3";
