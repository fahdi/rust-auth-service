    ...
    let configs = vec![
        ...
        (
            ...
            CacheConfig {
                ...
                ttl: 3600,
                lru_size: 1000,
                redis: Some(RedisConfig { url: "redis://localhost:6379".to_string(), ssl: false }),
                memory: MemoryConfig { max_size: 1000 },
            },
        ),
        (
...
async fn test_cache_configuration(config: &CacheConfig) -> Result<()> {
    ...
    cache
        .set_with_ttl("ttl_key", "ttl_value", Duration::from_millis(100))
        .await?;
...
async fn performance_test() -> Result<()> {
    ...
    for (name, config) in configs {
        if let Ok(provider) = create_cache_provider(&config).await {
            ...
            info!(
                "  Write: {:.0} ops/sec ({:.2}ms avg)",
                write_ops_per_sec,
                write_duration.as_millis() as f64 / OPERATIONS as f64
            );
            info!(
                "  Read:  {:.0} ops/sec ({:.2}ms avg)",
                read_ops_per_sec,
                read_duration.as_millis() as f64 / OPERATIONS as f64
            );

            // Test cache statistics
            let stats = cache.stats().await?;
            info!(
                "  Final stats - Hits: {}, Misses: {}, Size: {}, Hit Rate: {:.1}%",
                stats.hits,
                stats.misses,
                stats.size,
                stats.hit_rate * 100.0
            ...
            info!(
                "{} Cache: Connection failed, skipping performance test",
                name
            );
        }
    }
