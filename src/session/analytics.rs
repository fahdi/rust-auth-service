...
impl<T: SessionService> SessionAnalyticsService<T> {
    ...
    pub async fn generate_analytics_report(
        ...
        let geographic_data = self
            .analyze_geographic_distribution(start_date, end_date)
            .await?;
    ...
    async fn calculate_login_trends(
        &self,
        ...
        let hourly_distribution = self
            .calculate_hourly_distribution(start_date, end_date)
            .await?;
    ...
    async fn calculate_daily_counts(
        &self,
        start_date: DateTime<Utc>,
        end_date: DateTime<Utc>,
    ...
    fn identify_peak_hours(&self, hourly_distribution: &HashMap<u32, u32>) -> Vec<u32> {
        let mut hours_with_counts: Vec<(u32, u32)> = hourly_distribution
            .iter()
...
impl<T: SessionService> AnalyticsExporter<T> {
    ...
    pub async fn export_csv(&self, period: AnalyticsPeriod) -> Result<String> {
        let report = self
            .analytics_service
            .generate_analytics_report(period)
            .await?;
