...
use rust_auth_service::cache::{
    ...
    create_cache_provider, create_multi_level_cache
};
...
fn create_test_cache_configs() -> Vec<(String, CacheConfig)> {
    ...
    configs.push(("memory".to_string(), CacheConfig {
    ...
    configs.push(("none".to_string(), CacheConfig {
    ...
    if let Ok(redis_url) = std::env::var("REDIS_TEST_URL") {
        configs.push(("redis".to_string(), CacheConfig {
    ...
    if let Ok(redis_url) = std::env::var("REDIS_TEST_URL") {
        configs.push(("multi".to_string(), CacheConfig {
...
async fn test_basic_cache_operations() {
    ...
    for (cache_type, config) in configs {
        ...
        if cache_type == "none" {
            ...
            assert_eq!(retrieved, Some(value.to_string()), "{} should return stored value", cache_type);
        }
        ...
        assert_eq!(after_delete, None, "{} should return None after delete", cache_type);
...
async fn test_cache_ttl() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        cache.set(&key, value, Duration::from_millis(100)).await
            .expect("Set with TTL should succeed");
        
        // Should be available immediately
        let immediate = cache.get(&key).await
            .expect("Immediate get should succeed");
        assert_eq!(immediate, Some(value.to_string()), "{} should return value immediately", cache_type);
        
        // Wait for TTL to expire
        sleep(Duration::from_millis(150)).await;
        
        // Should be expired now
        let after_ttl = cache.get(&key).await
            .expect("Get after TTL should succeed");
        
        // Note: Memory cache with cleanup might still have the value until next cleanup
        // Redis should definitely have expired it
        if cache_type == "redis" {
            assert_eq!(after_ttl, None, "{} should return None after TTL", cache_type);
        }
...
async fn test_cache_health_checks() {
    ...
    for (cache_type, config) in configs {
        ...
        assert!(stats.hit_rate >= 0.0 && stats.hit_rate <= 1.0, 
               "{} hit rate should be between 0 and 1", cache_type);
...
async fn test_cache_statistics() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        assert!(stats.total_operations() > 0, "{} should have recorded operations", cache_type);
        ...
        assert!((stats.hit_ratio() - expected_ratio).abs() < 0.001, 
               "{} hit ratio calculation should be correct", cache_type);
        
        println!("✅ {} statistics passed (Hit ratio: {:.2}%)", 
                cache_type, stats.hit_ratio() * 100.0);
    }
}
...
async fn test_cache_service() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        service.set(&key, "service_value").await
            .expect("Service set should succeed");
        
        let value = service.get(&key).await
            .expect("Service get should succeed");
        assert_eq!(value, Some("service_value".to_string()), 
                  "{} service should return stored value", cache_type);
        
        // Test set with custom TTL
        let ttl_key = generate_test_key(&format!("service_ttl_{}", cache_type));
        service.set_with_ttl(&ttl_key, "ttl_value", Duration::from_secs(30)).await
            .expect("Service set with TTL should succeed");
        
        let ttl_value = service.get(&ttl_key).await
            .expect("Service get TTL value should succeed");
        assert_eq!(ttl_value, Some("ttl_value".to_string()), 
                  "{} service should return TTL value", cache_type);
        
        // Test delete
        service.delete(&key).await
            .expect("Service delete should succeed");
        
        let after_delete = service.get(&key).await
            .expect("Service get after delete should succeed");
        assert_eq!(after_delete, None, "{} service should return None after delete", cache_type);
...
async fn test_get_or_set_pattern() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        let value1 = service.get_or_set(&key, || {
            compute_count += 1;
            async move { Ok("computed_value".to_string()) }
        }).await.expect("Get or set should succeed");
        
        assert_eq!(value1, "computed_value", "{} should return computed value", cache_type);
        assert_eq!(compute_count, 1, "{} should have computed once", cache_type);
        
        // Second call should use cached value
        let value2 = service.get_or_set(&key, || {
            compute_count += 1;
            async move { Ok("should_not_compute".to_string()) }
        }).await.expect("Get or set should succeed");
        
        assert_eq!(value2, "computed_value", "{} should return cached value", cache_type);
        assert_eq!(compute_count, 1, "{} should not have computed again", cache_type);
        
        // Test get_or_set with custom TTL
        let ttl_key = generate_test_key(&format!("get_or_set_ttl_{}", cache_type));
        let ttl_value = service.get_or_set_with_ttl(&ttl_key, Duration::from_secs(30), || async {
            Ok("ttl_computed_value".to_string())
        }).await.expect("Get or set with TTL should succeed");
        
        assert_eq!(ttl_value, "ttl_computed_value", 
                  "{} should return TTL computed value", cache_type);
...
async fn test_cache_key_utilities() {
    ...
    assert_eq!(CacheKey::password_reset_token("reset_token_123"), "reset_token:reset_token_123");
    assert_eq!(CacheKey::email_verification_token("verify_token_123"), "verify_token:verify_token_123");
    assert_eq!(CacheKey::rate_limit("192.168.1.1"), "rate_limit:192.168.1.1");
    assert_eq!(CacheKey::login_attempts("user@example.com"), "login_attempts:user@example.com");
    ...
    cache.set(&user_key, user_data, Duration::from_secs(60)).await
        .expect("User cache set should succeed");
    
    let cached_user = cache.get(&user_key).await
        .expect("User cache get should succeed");
    assert_eq!(cached_user, Some(user_data.to_string()), "Cached user data should match");
    ...
    cache.set(&session_key, session_data, Duration::from_secs(60)).await
        .expect("Session cache set should succeed");
    
    let cached_session = cache.get(&session_key).await
        .expect("Session cache get should succeed");
    assert_eq!(cached_session, Some(session_data.to_string()), "Cached session data should match");
...
async fn test_multi_level_cache() {
    ...
    let cache = create_multi_level_cache(&config).await
    ...
    assert_eq!(retrieved, Some(value.to_string()), "Multi-level cache should return stored value");
    
    // Test failover behavior by ensuring fallback works
    let stats = cache.stats().await
        .expect("Multi-level stats should be available");
    assert!(stats.total_operations() >= 0, "Multi-level cache should have operation stats");
    
    // Test ping for health check
    cache.ping().await
        .expect("Multi-level ping should succeed");
...
async fn test_concurrent_cache_operations() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        println!("📊 {} concurrent sets - Success: {}/{}", cache_type, successful, CONCURRENT_OPS);
        assert!(successful >= CONCURRENT_OPS * 8 / 10, 
               "{} should handle concurrent operations (>80% success rate)", cache_type);
        ...
        println!("📊 {} concurrent gets - Success: {}/{}", cache_type, get_successful, CONCURRENT_OPS);
...
async fn test_cache_error_handling() {
    ...
    for (cache_type, config) in configs {
        ...
        assert!(empty_result.is_ok(), "{} should handle empty key gracefully", cache_type);
        ...
        assert!(long_key_result.is_ok(), "{} should handle long keys", cache_type);
        
        // Test operations with special characters in key
        let special_key = "key:with/special\\chars@#$%";
        let special_result = cache.set(special_key, "value", Duration::from_secs(60)).await;
        assert!(special_result.is_ok(), "{} should handle special characters", cache_type);
        
        // Test zero TTL
        let zero_ttl_result = cache.set("zero_ttl", "value", Duration::from_secs(0)).await;
        assert!(zero_ttl_result.is_ok(), "{} should handle zero TTL", cache_type);
...
async fn test_cache_memory_management() {
    ...
    cache.set("key3", "value3", Duration::from_secs(60)).await
    ...
    cache.set("key4", "value4", Duration::from_secs(60)).await
...
async fn test_cache_performance() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        for i in 0..PERFORMANCE_OPS {
            ...
            cache.set(&key, "performance_value", Duration::from_secs(60)).await
        ...
        println!("   Set: {:.0} ops/sec ({:.2}ms avg)", set_ops_per_sec, set_duration.as_millis() as f64 / PERFORMANCE_OPS as f64);
        println!("   Get: {:.0} ops/sec ({:.2}ms avg)", get_ops_per_sec, get_duration.as_millis() as f64 / PERFORMANCE_OPS as f64);
        
        // Performance thresholds (very lenient for testing environments)
        assert!(set_ops_per_sec > 10.0, "{} set performance should be reasonable", cache_type);
        assert!(get_ops_per_sec > 50.0, "{} get performance should be reasonable", cache_type);
...
async fn test_complete_cache_workflow() {
    ...
    for (cache_type, config) in configs.into_iter().filter(|(t, _)| t != "none") {
        ...
        service.set(&login_attempts_key, "1").await
        ...
        service.set_with_ttl(&reset_key, user_id, Duration::from_secs(1800)).await
        ...
        service.set_with_ttl(&rate_limit_key, "5", Duration::from_secs(300)).await
            .expect("Rate limit caching should succeed");
        
        // Step 5: Verify all data is cached correctly
        assert_eq!(service.get(&user_key).await.unwrap(), Some(user_data.to_string()));
        assert_eq!(service.get(&session_key).await.unwrap(), Some(session_data.to_string()));
        assert_eq!(service.get(&login_attempts_key).await.unwrap(), Some("1".to_string()));
        assert_eq!(service.get(&reset_key).await.unwrap(), Some(user_id.to_string()));
        assert_eq!(service.get(&rate_limit_key).await.unwrap(), Some("5".to_string()));
        
        // Step 6: Test get_or_set for computed values
        let computed_key = CacheKey::user_by_email(email);
        let computed_value = service.get_or_set(&computed_key, || async {
        ...
        service.delete(&session_key).await
            .expect("Session cleanup should succeed");
        service.delete(&reset_key).await
        ...
        assert!(stats.total_operations() > 0, "Cache should have recorded operations");
        
        println!("✅ {} complete workflow passed", cache_type);
        println!("   Final stats - Hits: {}, Misses: {}, Hit Rate: {:.1}%", 
