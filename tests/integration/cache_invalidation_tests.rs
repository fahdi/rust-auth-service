...
use rust_auth_service::cache::{CacheProvider, CacheKey};
...
mod invalidation_integration {
    ...
    async fn test_user_profile_cache_invalidation() -> Result<()> {
        ...
        let original_profile = r#"{"id":"user_123","name":"John Doe","email":"test@example.com","role":"user"}"#;
        let updated_profile = r#"{"id":"user_123","name":"John Smith","email":"test@example.com","role":"admin"}"#;
        
        // Cache user profile data
        cache.set_and_track(&user_profile_key, original_profile, Duration::from_secs(300)).await?;
        cache.set_and_track(&user_email_key, original_profile, Duration::from_secs(300)).await?;
        
        // Verify data is cached
        assert_eq!(cache.provider.get(&user_profile_key).await?, Some(original_profile.to_string()));
        assert_eq!(cache.provider.get(&user_email_key).await?, Some(original_profile.to_string()));
        ...
        cache.provider.set(&user_profile_key, updated_profile, Duration::from_secs(300)).await?;
        cache.provider.set(&user_email_key, updated_profile, Duration::from_secs(300)).await?;
        
        // Verify updated data is cached
        assert_eq!(cache.provider.get(&user_profile_key).await?, Some(updated_profile.to_string()));
        assert_eq!(cache.provider.get(&user_email_key).await?, Some(updated_profile.to_string()));
    ...
    async fn test_session_cache_invalidation() -> Result<()> {
        ...
        // Cache session data
        cache.set_and_track(&session_key, session_data, Duration::from_secs(3600)).await?;
        
        // Verify session is cached
        assert_eq!(cache.provider.get(&session_key).await?, Some(session_data.to_string()));
    ...
    async fn test_jwt_token_blacklisting() -> Result<()> {
        ...
        for token in &tokens {
            ...
            cache.set_and_track(&blacklist_key, "revoked", Duration::from_secs(86400)).await?; // 24 hours
        }
    ...
    async fn test_rate_limit_cache_invalidation() -> Result<()> {
        ...
        for ip in &client_ips {
            ...
            let rate_limit_data = r#"{"requests":5,"window_start":"2025-10-18T01:00:00Z","limit":100}"#;
            
            // Cache rate limit data with window TTL
            cache.set_and_track(&rate_limit_key, rate_limit_data, Duration::from_secs(60)).await?;
        }
    ...
    async fn test_password_reset_token_invalidation() -> Result<()> {
        ...
        for token in &reset_tokens {
            ...
            let token_data = r#"{"user_id":"user_123","expires_at":"2025-10-18T02:00:00Z","used":false}"#;
            
            // Cache reset token with expiration
            cache.set_and_track(&token_key, token_data, Duration::from_secs(3600)).await?;
        }
        ...
        for token in &reset_tokens[0..2] { // Use first two tokens
            let token_key = CacheKey::password_reset_token(token);
    ...
    async fn test_email_verification_token_invalidation() -> Result<()> {
        ...
        for token in &verification_tokens {
            ...
            cache.set_and_track(&token_key, token_data, Duration::from_secs(86400)).await?;
        }
    ...
    async fn test_login_attempts_cache_invalidation() -> Result<()> {
        ...
        let user_emails = vec!["user1@example.com", "user2@example.com", "user3@example.com"];
        
        for email in &user_emails {
            let attempts_key = CacheKey::login_attempts(email);
            let attempts_data = r#"{"count":3,"last_attempt":"2025-10-18T01:30:00Z","locked_until":null}"#;
            
            // Cache login attempts
            cache.set_and_track(&attempts_key, attempts_data, Duration::from_secs(1800)).await?;
        }
    ...
    async fn test_cache_warming_after_invalidation() -> Result<()> {
        ...
        for (i, user) in users.iter().enumerate() {
            ...
            cache.set_and_track(&user_key, &user_data, Duration::from_secs(300)).await?;
        ...
        for (i, user) in users.iter().enumerate() {
            ...
            let user_data = serde_json::to_string(&user).unwrap();
            cache.provider.set(&user_key, &user_data, Duration::from_secs(300)).await?;
    ...
    async fn test_time_based_invalidation() -> Result<()> {
        ...
        assert_eq!(cache.provider.get("medium_lived").await?, Some("expires_later".to_string()));
        assert_eq!(cache.provider.get("long_lived").await?, Some("expires_much_later".to_string()));
