...
async fn test_comprehensive_authentication_system() -> Result<()> {
    ...
    assert!(
        auth_flow_result.register_time < Duration::from_millis(2000),
        "Registration should complete in <2000ms"
    );
    assert!(
        auth_flow_result.login_time < Duration::from_millis(1500),
        "Login should complete in <1500ms"
    );
    assert!(
        auth_flow_result.profile_time < Duration::from_millis(1000),
    ...
    println!(
        "     Registration: {:.2}ms",
        auth_flow_result.register_time.as_millis()
    );
    println!(
        "     Login: {:.2}ms",
        auth_flow_result.login_time.as_millis()
    );
    println!(
        "     Profile: {:.2}ms",
        auth_flow_result.profile_time.as_millis()
    );
    println!(
        "     Total: {:.2}ms",
        auth_flow_result.total_time.as_millis()
    );
    ...
    for (endpoint, status) in &protected_endpoints_result.unauthorized_access {
        assert_eq!(
            *status,
            ...
            "Endpoint {} should return 401 for unauthorized access",
            endpoint
        );
    }
    ...
    for (endpoint, status) in &protected_endpoints_result.invalid_token_access {
        assert_eq!(
            *status,
            reqwest::StatusCode::UNAUTHORIZED,
            "Endpoint {} should return 401 for invalid tokens",
            endpoint
        );
    }

    println!(
        "✅ Access control working correctly on {} endpoints",
    ...
    assert!(
        concurrent_successes >= (concurrent_users * 8 / 10),
        "At least 80% of concurrent operations should succeed"
    );
    println!(
        "✅ Concurrent operations: {}/{} successful ({:.1}%)",
    ...
    for weak_password in weak_passwords {
        let weak_user =
            TestUser::with_custom_data("test@example.com", weak_password, "Test", "User");
    ...
    assert!(
        load_results.operations_per_second > 5.0,
        "Should handle at least 5 operations per second under load"
    );
    assert!(
        load_results.successful_operations as f64 / load_results.total_operations as f64 > 0.85,
    ...
    println!(
        "     Operations/second: {:.2}",
        load_results.operations_per_second
    );
    println!(
        "     Success rate: {:.1}%",
    ...
    println!(
        ...
        load_results.average_response_time.as_millis()
    );
    ...
    let wrong_password_user = TestUser::with_custom_data(
        ...
        "WrongPassword123!",
        &duplicate_user.first_name,
        &duplicate_user.last_name,
    );
...
async fn test_authentication_performance_benchmark() -> Result<()> {
    ...
    let avg_registration =
        registration_times.iter().sum::<Duration>() / registration_times.len() as u32;
    let avg_login = login_times.iter().sum::<Duration>() / login_times.len() as u32;

    println!("📊 Performance Benchmark Results:");
    println!(
        "  Average Registration Time: {:.2}ms",
        avg_registration.as_millis()
    );
    println!("  Average Login Time: {:.2}ms", avg_login.as_millis());

    // Performance assertions (adjust thresholds as needed)
    assert!(
        avg_registration < Duration::from_millis(2000),
        "Average registration time should be <2000ms"
    );
    assert!(
        avg_login < Duration::from_millis(1000),
