...
mod memory_integration {
    ...
    async fn test_memory_lru_eviction() -> Result<()> {
        ...
        cache.set_and_track("key1", "value1", Duration::from_secs(300)).await?;
        cache.set_and_track("key2", "value2", Duration::from_secs(300)).await?;
        cache.set_and_track("key3", "value3", Duration::from_secs(300)).await?;
        
        // All should be present
        assert_eq!(cache.provider.get("key1").await?, Some("value1".to_string()));
        assert_eq!(cache.provider.get("key2").await?, Some("value2".to_string()));
        assert_eq!(cache.provider.get("key3").await?, Some("value3".to_string()));
        
        // Add one more to trigger eviction of least recently used (key1)
        cache.set_and_track("key4", "value4", Duration::from_secs(300)).await?;
        
        // key1 should be evicted
        assert_eq!(cache.provider.get("key1").await?, None);
        assert_eq!(cache.provider.get("key2").await?, Some("value2".to_string()));
        assert_eq!(cache.provider.get("key3").await?, Some("value3".to_string()));
        assert_eq!(cache.provider.get("key4").await?, Some("value4".to_string()));
        ...
        cache.set_and_track("key5", "value5", Duration::from_secs(300)).await?;
        
        assert_eq!(cache.provider.get("key2").await?, Some("value2".to_string()));
        assert_eq!(cache.provider.get("key3").await?, None);
        assert_eq!(cache.provider.get("key4").await?, Some("value4".to_string()));
        assert_eq!(cache.provider.get("key5").await?, Some("value5".to_string()));
    ...
    async fn test_memory_capacity_limits() -> Result<()> {
        ...
        assert_eq!(present_count, 5, "Cache should maintain exactly capacity limit");
    ...
    async fn test_memory_ttl_cleanup() -> Result<()> {
        ...
        cache.set_and_track("short_ttl", "short_value", Duration::from_millis(50)).await?;
        cache.set_and_track("medium_ttl", "medium_value", Duration::from_millis(200)).await?;
        cache.set_and_track("long_ttl", "long_value", Duration::from_secs(300)).await?;
        
        // All should be present initially
        assert_eq!(cache.provider.get("short_ttl").await?, Some("short_value".to_string()));
        assert_eq!(cache.provider.get("medium_ttl").await?, Some("medium_value".to_string()));
        assert_eq!(cache.provider.get("long_ttl").await?, Some("long_value".to_string()));
    ...
    async fn test_memory_performance_characteristics() -> Result<()> {
        ...
        for i in 0..100 {
            ...
            cache.provider.set(&key, &value, Duration::from_secs(60)).await?;
        ...
        assert!(set_ops_per_sec > 1000.0, "Memory cache sets should be >1000 ops/sec");
        assert!(get_ops_per_sec > 5000.0, "Memory cache gets should be >5000 ops/sec");
    ...
    async fn test_memory_stress_testing() -> Result<()> {
        ...
        assert!(success_rate > 0.98, "Memory cache should handle stress with >98% success rate");
        assert!(ops_per_second > 500.0, "Memory cache should handle >500 ops/sec under stress");
    ...
    async fn test_memory_zero_capacity_edge_case() -> Result<()> {
        ...
        cache.set("single_key", "single_value", Duration::from_secs(60)).await?;
        let value = cache.get("single_key").await?;
        assert_eq!(value, Some("single_value".to_string()));
        
        // Adding another should evict the first
        cache.set("second_key", "second_value", Duration::from_secs(60)).await?;
        
        assert_eq!(cache.get("single_key").await?, None);
        assert_eq!(cache.get("second_key").await?, Some("second_value".to_string()));
    ...
    async fn test_memory_update_existing_keys() -> Result<()> {
        ...
        // Set initial value
        cache.set_and_track("update_key", "initial_value", Duration::from_secs(60)).await?;
        assert_eq!(cache.provider.get("update_key").await?, Some("initial_value".to_string()));
        
        // Update with new value
        cache.provider.set("update_key", "updated_value", Duration::from_secs(60)).await?;
        assert_eq!(cache.provider.get("update_key").await?, Some("updated_value".to_string()));
        
        // Update with different TTL
        cache.provider.set("update_key", "final_value", Duration::from_millis(100)).await?;
        assert_eq!(cache.provider.get("update_key").await?, Some("final_value".to_string()));
