...
mod multilevel_integration {
    ...
    async fn test_multilevel_stress_testing() -> Result<()> {
        ...
        // Run stress test
        // let stress_runner = StressTestRunner::new(15, 150);  // Temporarily disabled due to model mismatches
    ...
    async fn test_multilevel_health_checks() -> Result<()> {
        ...
        for i in 0..5 {
            // let (_, _metrics) = measure_async("multilevel_ping", "multilevel", cache.provider.ping()).await?;  // Temporarily disabled
