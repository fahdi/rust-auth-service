...
use rust_auth_service::cache::{CacheProvider, MemoryCache, RedisCache, MultiLevelCache};
...
mod multilevel_integration {
    ...
    async fn test_multilevel_fallback_behavior() -> Result<()> {
        ...
        multi_cache.set("fallback_key", "fallback_value", Duration::from_secs(60)).await?;
    ...
    async fn test_multilevel_cache_promotion() -> Result<()> {
        ...
        let redis_cache = Arc::new(RedisCache::new(
            &std::env::var("REDIS_URL").unwrap_or_else(|_| "redis://localhost:6379".to_string())
        ).await?);
        let memory_cache = Arc::new(MemoryCache::new(100));
        let multi_cache = Arc::new(MultiLevelCache::new(Some(redis_cache.clone()), memory_cache.clone()));
        ...
        memory_cache.set("promotion_key", "promotion_value", Duration::from_secs(60)).await?;
        
        // Verify it's only in memory cache
        assert_eq!(memory_cache.get("promotion_key").await?, Some("promotion_value".to_string()));
    ...
    async fn test_multilevel_consistency() -> Result<()> {
        ...
        cache.set_and_track("consistency_key1", "value1", Duration::from_secs(60)).await?;
        cache.set_and_track("consistency_key2", "value2", Duration::from_secs(60)).await?;
        
        // Values should be consistently retrievable
        assert_eq!(cache.provider.get("consistency_key1").await?, Some("value1".to_string()));
        assert_eq!(cache.provider.get("consistency_key2").await?, Some("value2".to_string()));
        
        // Update values
        cache.provider.set("consistency_key1", "updated_value1", Duration::from_secs(60)).await?;
        
        // Should get updated value
        assert_eq!(cache.provider.get("consistency_key1").await?, Some("updated_value1".to_string()));
        assert_eq!(cache.provider.get("consistency_key2").await?, Some("value2".to_string()));
    ...
    async fn test_multilevel_statistics_aggregation() -> Result<()> {
        ...
        cache.set_and_track("stats_key1", "value1", Duration::from_secs(60)).await?;
        cache.set_and_track("stats_key2", "value2", Duration::from_secs(60)).await?;
    ...
    async fn test_multilevel_ttl_behavior() -> Result<()> {
        ...
        // Set with short TTL
        cache.set_and_track("ttl_key", "ttl_value", Duration::from_millis(100)).await?;
        
        // Should be available immediately
        assert_eq!(cache.provider.get("ttl_key").await?, Some("ttl_value".to_string()));
    ...
    async fn test_multilevel_error_resilience() -> Result<()> {
        ...
        multi_cache.set("resilience_key", "resilience_value", Duration::from_secs(60)).await?;
    ...
    async fn test_multilevel_performance_characteristics() -> Result<()> {
        ...
        assert!(set_ops_per_sec > 100.0, "Multi-level cache sets should be >100 ops/sec");
        assert!(get_ops_per_sec > 500.0, "Multi-level cache gets should be >500 ops/sec");
    ...
    async fn test_multilevel_stress_testing() -> Result<()> {
        ...
        assert!(success_rate > 0.95, "Multi-level cache should handle stress with >95% success rate");
        assert!(ops_per_second > 100.0, "Multi-level cache should handle >100 ops/sec under stress");
    ...
    async fn test_multilevel_cache_warming() -> Result<()> {
        ...
        let warm_data = vec![
            ("warm_user:1", r#"{"id":1,"name":"User1","email":"user1@example.com"}"#),
            ("warm_user:2", r#"{"id":2,"name":"User2","email":"user2@example.com"}"#),
            ("warm_config", r#"{"theme":"dark","lang":"en"}"#),
            ("warm_permissions", r#"{"admin":true,"read":true,"write":false}"#),
        ];
        ...
        assert!(avg_access_time < 10.0, "Warmed cache access should be <10ms per item");
