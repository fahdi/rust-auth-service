...
-- Note: CREATE EVENT statements are not supported in prepared statements
-- Events should be created manually in production if needed:
...
--     WHERE email_verification_expires IS NOT NULL AND email_verification_expires < NOW();
...
-- DROP EVENT IF EXISTS cleanup_expired_tokens; -- Not created in this migration
-- MySQL syntax for dropping indexes
DROP INDEX idx_users_reset_token_expires ON users;
DROP INDEX idx_users_verification_token_expires ON users;
DROP INDEX idx_users_role_active ON users;
DROP INDEX idx_users_email_active ON users;
DROP INDEX idx_users_locked ON users;
...
DROP INDEX idx_users_user_id ON users;
